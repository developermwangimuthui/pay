...
/*		    echo '<pre>';echo print_r($alertSetting);exit;*/
		//setting
		$setting = $this->setting();
		$existUser->app_name = $setting[18]->value;

		if($alertSetting[0]->forgot_email==1 and !empty($existUser->email)){
			/*Mail::send('/mail/recoverPassword', ['existUser' => $existUser], function($m) use ($existUser){
				$m->to($existUser->email)->subject(Lang::get("labels.fogotPasswordEmailTitle"))->getSwiftMessage()
				->getHeaders()
				->addTextHeader('x-mailgun-native-send', 'true');
			});*/

            $email = $existUser->email;
            $emailmessage = 'Hi, '.$existUser->first_name.' '.$existUser->last_name.'! <br>Your New credentials are given:<br> username: '.$existUser->email.'<br> New Passowrd: '.$existUser->password.'<br>Please Use the above credentials to login the Pfaccounts Portal <br> Thank you!';
            $emailsubject = 'Recover Password';

			$mail = $this->sendEmail($email,$emailmessage,$emailsubject);
			return $mail;
			/*if($mail) {
			    return redirect('admin/login')->with('loginError', Lang::get("Email Sent to your mail Address"));
            }else{
                return redirect('admin/login')->with('loginError', Lang::get("Email Sent to your mail Address"));
		...
		/*if($alertSetting[0]->forgot_notification==1){
			...
			//get device id
			$device_id = $this->userDevice($existUser->customers_id);
			if(!empty($device_id)){
				$response = $this->$functionName($device_id, $sendData);
			}
		}*/
